...
`asciimaze/fixed4x4/finetune/lora/`) and converts to GGUF for use with llama.cpp.
...
Defaults to `asciimaze/fixed4x4/finetune/gguf/`, keeping all finetune artifacts
(lora / merged / gguf) nested under one `asciimaze/fixed4x4/finetune/` project folder.

No argparse flag here carries a literal default - every value comes from the
"gguf:" section of `asciimaze/fixed4x4/config.yaml` (auto-loaded via
...
def parse_args():
    ...
    parser.add_argument("--config", default="asciimaze/fixed4x4/config.yaml",
                         help="Path to the shared YAML config (see asciimaze/fixed4x4/config.yaml). "
                              "This script reads the top-level 'gguf:' section - keys match the "
                              "long flag names with dashes replaced by underscores (e.g. lora_path, "
                              "gguf_dir). Any flag also passed on the command line overrides the "
                              "config file value.")
    ...
    parser.add_argument("--quantization-method",
    ...
    # Only this script's "gguf:" section is used; ${experiment_dir} in any
