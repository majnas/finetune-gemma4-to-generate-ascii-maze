# -*- coding: utf-8 -*-
"""
Gemma-4 (E2B) - generate N samples from the MERGED float16 model.

Same as `gemma4_generate_100_samples_with_lora.py`, but loads the standalone
merged model produced by `gemma4_export.py --save-merged` (base weights +
adapter already fused into one float16 checkpoint) instead of base+adapter.
...
history carried over, so the N outputs are independent samples. Every output is
written to a single output file (one delimited block per sample, flushed after
each so progress survives an interrupt).
...
def parse_args():
    ...
    # Model loading
    parser.add_argument("--model-path", default="asciimaze/fixed4x4/finetune/merged",
                         help="Path to the merged float16 model saved by "
                              "gemma4_export.py --save-merged")
    parser.add_argument("--max-seq-length", type=int, default=2048)
    # Merged model defaults to FULL precision (its whole point). Pass
    # --load-in-4bit to load it quantized to save memory.
    ...
    parser.add_argument("--output-file", default="asciimaze/fixed4x4/outputs/merged_samples.txt",
...
model, tokenizer = FastModel.from_pretrained(
    model_name = args.model_path,  # merged float16 model from gemma4_export.py
